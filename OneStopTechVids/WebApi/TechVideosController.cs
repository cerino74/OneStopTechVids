...
namespace OneStopTechVids
{
    public class TechVideosController : ApiController
    {
        ...
        {
            var video =
                TechVideosData.TechVideos.FirstOrDefault(
                    tv => tv.Title.Equals(title, StringComparison.InvariantCultureIgnoreCase));
            if (video != null)
            {
                return Ok(false);
            }
